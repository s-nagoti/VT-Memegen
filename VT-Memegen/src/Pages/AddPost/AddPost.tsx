  ...
  const handleAddPost = async () => {
    ...
    for (let area of textAreas) {
      if (!textInputs[area.key]?.trim()) {
        setMessage({ type: 'error', text: `Please enter text for "${area.placeholder}".` });
        return;
      }
    }
    ...
    try {
      ...
      if (previewRef.current) {
        ...
        if (user?.id) {
          await updateDoc(doc(db, 'users', user.id), { posts: arrayUnion(docRef.id) });
        }
      ...
      setMessage({ type: 'error', text: 'Error adding post. Please try again.' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex-grow container mx-auto p-6">
        <div className="bg-white shadow-md rounded-lg p-8 max-w-4xl mx-auto">
              ...
              {imageTemplates.map((template, index) => (
                <div
                  ...
                  className={`flex-shrink-0 cursor-pointer relative ${
                    selectedTemplateIndex === index ? 'border-4 border-blue-500 rounded-lg' : 'border-2 border-transparent'
          ...
          {selectedTemplateIndex !== null && (
            <div className="mb-6">
              <h3 className="text-xl font-semibold mb-4 text-gray-800">Customize Your Template</h3>
                ...
                <div className="md:w-1/2">
                  {imageTemplates[selectedTemplateIndex].textAreas.map((area) => (
                    <div key={area.key} className="mb-4">
                      <label htmlFor={area.key} className="block text-gray-700 mb-2 font-medium">
                        {area.placeholder}
                      </label>
                      <input
                        id={area.key}
